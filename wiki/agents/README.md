...
- The orchestrator is the main session, not a subagent. In Claude Code it is the `orchestrator` agent, set as the default agent via `"agent": "orchestrator"` in `~/.claude/settings.json`, so plain `claude` starts as the orchestrator (Opus, medium effort). In Codex the main session already runs Sol at high effort; the Orchestrator mode section in `wiki/homeBase/AGENTS.md` points it to `roles/orchestrator.md` for the full procedure.
